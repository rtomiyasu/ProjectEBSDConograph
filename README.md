...
Since $`M^{new}`$ is a generalization of the de Wolff M used in powder indexing, $`M`$ and $`M^{new}`$ have similar properties, except that M prefers higher-symmetric cells ([Table](https://github.com/rtomiyasu/ProjectEBSDConograph/blob/main/figures/table5_2_en.png)). Namely,
